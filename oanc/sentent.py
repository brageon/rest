...
if __name__ == "__main__":
  bacl = BaCl() #5.4, 3.5; 2.8, 1.8, 3.4; 9.6; 12, 3.7, 2.7
  with open('ac.txt', 'r') as file:
    lines = file.readlines()
    for line in lines:
      hydra = bacl.remove_non(line)
      dan = bacl.translate(hydra)
      mid = bacl.count_trigrams(dan)
      ouro, sun, varm, zs, sd = bacl.calc(dan)
      percent_scores = []
      av, am, wc = 0, 0, 15
      gt = round(sun / ouro, 2)
      for trigram, count in mid.items():
        ...
        if len(percent_scores) > 0:
          av = len(percent_scores) / sum(percent_scores)
          am = sum(percent_scores) / len(percent_scores)
        ...
        zeta = wc - (gt / zs)
        zat = am / gt
        ares = (av / zeta) - (zeta / am)
        epok = (zeta / av) - (am / zeta)
        sac = (ares - epok) / 2
        ...
        bo = (safe, av, am, sac, zat, crow1, so)
